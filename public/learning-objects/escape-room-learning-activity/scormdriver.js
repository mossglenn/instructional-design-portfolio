...
function SCORM2004_GetInteractionTimestamp(strInteractionID) {
  WriteToDebug(
    'In SCORM2004_GetInteractionTimestamp, strInteractionID=' +
      strInteractionID,
  );
  var intInteractionIndex =
...
function SCORM2004_GetInteractionCorrectResponses(strInteractionID) {
  ...
  if (intCorrectResponseCount == 0) return [];
...
function SCORM2004_GetInteractionWeighting(strInteractionID) {
  WriteToDebug(
    'In SCORM2004_GetInteractionWeighting, strInteractionID=' +
      strInteractionID,
  );
  var intInteractionIndex =
    SCORM2004_FindInteractionIndexFromID(strInteractionID);
...
function SCORM2004_CallSetValue(strElement, strValue) {
  var strResult;
  WriteToDebug(
    'SCORM2004_CallSetValue strElement=' +
      strElement +
      ', strValue=' +
      strValue,
  );
...
var AICC_aryObjectivesRead = [];
var AICC_aryObjectivesWrite = [];
var AICC_aryCommentsFromLearner = [];
var AICC_aryInteractions = [];
...
function AICC_GetMaxTimeAllowed() {
  WriteToDebug(
    'In AICC_GetMaxTimeAllowed, Returning ' +
      AICC_intMaxTimeAllowedMilliseconds,
  );
  return AICC_intMaxTimeAllowedMilliseconds;
}
function AICC_DisplayMessageOnTimeout() {
  WriteToDebug(
    'In AICC_DisplayMessageOnTimeout, Returning ' +
...
function AICC_TranslateTimeLimitAction(strTimeLimitAction) {
  WriteToDebug(
    'In AICC_TranslateTimeLimitAction, strTimeLimitAction=' +
      strTimeLimitAction,
  );
  var arySplit;
  var blnError = false;
    ...
    parse: function (base64Str) {
      ...
      for (var i = 0; i < base64StrLength; i++) {
        if (i % 4) {
          ...
          words[nBytes >>> 2] |=
            (bitsHigh | bitsLow) << (24 - (nBytes % 4) * 8);
          nBytes++;
        }
      }
  ...
  LRS.prototype = {
    ...
    retrieveStatement: function (stmtId, cfg) {
      ...
      if (!callbackWrapper) {
        ...
        if (requestResult.err === null) {
          requestResult.statement = TinCan.Statement.fromJSON(
            requestResult.xhr.responseText,
          );
        }
      }
      ...
      if (!callbackWrapper) {
        ...
        if (requestResult.err === null && requestResult.xhr.status !== 404) {
          ...
          if (
            ...
            requestResult.state.etag = TinCan.Utils.getSHA1String(
              requestResult.xhr.responseText,
            );
          }
      ...
      if (!callbackWrapper) {
        ...
        if (requestResult.err === null && requestResult.xhr.status !== 404) {
          ...
          if (
            ...
            requestResult.profile.etag = TinCan.Utils.getSHA1String(
              requestResult.xhr.responseText,
            );
          }
          if (typeof requestResult.xhr.contentType !== 'undefined') {
...
function TCAPI_RecordInteraction(
  ...
  if (actObj.id !== null) {
    ...
    if (strResponse !== null) {
      stmt.result = { response: strResponse, success: blnCorrect };
    }
    tcapi_cache.statementQueue.push(stmt);
  }
  return true;
...
function TCAPI_GetBucketInfo(strBucketId) {
  WriteToDebug(
    'AICC_DetectSSPSupport - TCAPI does not support SSP, returning empty SSPBucketSize',
  );
  return new SSPBucketSize(0, 0);
}
...
function TCAPI_IsContentInBrowseMode() {
  WriteToDebug(
    'In TCAPI_IsContentInBrowseMode - TCAPI does not support BrowseMode',
  );
  return false;
}
function TCAPI_FindInteractionIndexFromID(strInteractionID) {
  WriteToDebug(
    'TCAPI_FindInteractionIndexFromID - TCAPI does not support interaction retrieval, returning null',
  );
  return null;
}
...
function LMSStandardAPI(strStandard) {
  ...
  eval(
    'this.GetAudioPlayPreference = ' + strStandard + '_GetAudioPlayPreference',
  );
  eval(
    'this.GetAudioVolumePreference = ' +
      strStandard +
      '_GetAudioVolumePreference',
  );
  eval('this.SetAudioPreference = ' + strStandard + '_SetAudioPreference');
  eval(
    'this.SetLanguagePreference = ' + strStandard + '_SetLanguagePreference',
  );
  eval(
    'this.GetLanguagePreference = ' + strStandard + '_GetLanguagePreference',
  );
  ...
  eval(
    'this.RecordTrueFalseInteraction = ' +
      strStandard +
      '_RecordTrueFalseInteraction',
  );
  eval(
    'this.RecordMultipleChoiceInteraction = ' +
      strStandard +
      '_RecordMultipleChoiceInteraction',
  );
  eval(
    'this.RecordFillInInteraction = ' +
      strStandard +
      '_RecordFillInInteraction',
  );
  eval(
    'this.RecordMatchingInteraction = ' +
      strStandard +
      '_RecordMatchingInteraction',
  );
  eval(
    'this.RecordPerformanceInteraction = ' +
      strStandard +
      '_RecordPerformanceInteraction',
  );
  eval(
    'this.RecordSequencingInteraction = ' +
      strStandard +
      '_RecordSequencingInteraction',
  );
  eval(
    'this.RecordLikertInteraction = ' +
      strStandard +
      '_RecordLikertInteraction',
  );
  eval(
    'this.RecordNumericInteraction = ' +
      strStandard +
      '_RecordNumericInteraction',
  );
  ...
  eval(
    'this.SetObjectiveDescription = ' +
      strStandard +
      '_SetObjectiveDescription',
  );
  eval(
    'this.GetObjectiveDescription = ' +
      strStandard +
  ...
  eval(
    'this.GetInteractionTimestamp = ' +
      strStandard +
      '_GetInteractionTimestamp',
  );
  eval(
    'this.GetInteractionCorrectResponses = ' +
      strStandard +
      '_GetInteractionCorrectResponses',
  );
  eval(
    'this.GetInteractionWeighting = ' +
      strStandard +
      '_GetInteractionWeighting',
  );
  eval(
    'this.GetInteractionLearnerResponses = ' +
      strStandard +
      '_GetInteractionLearnerResponses',
  );
  eval('this.GetInteractionResult = ' + strStandard + '_GetInteractionResult');
  eval(
    'this.GetInteractionLatency = ' + strStandard + '_GetInteractionLatency',
  );
  eval(
  ...
  eval(
    'this.SetObjectiveProgressMeasure = ' +
      strStandard +
      '_SetObjectiveProgressMeasure',
  );
  eval(
    'this.GetObjectiveProgressMeasure = ' +
      strStandard +
      '_GetObjectiveProgressMeasure',
  );
      ...
      if (strTemp != null && strTemp != '') {
        ...
        if (!(typeof objTempAPI == 'undefined' || objTempAPI == null)) {
          ...
          if (!(typeof objTempAPI == 'undefined' || objTempAPI == null)) {
            ...
            if (strTemp !== null && strTemp !== '') {
              WriteToDebug(
                "Found TCAPI via 'endpoint' query string param, using TCAPI",
              );
              objLMS = new LMSStandardAPI('TCAPI');
              blnLmsPresent = true;
              ...
              if (ALLOW_NONE_STANDARD === true) {
                ...
                DisplayError(
                  'Could not determine standard. Neither SCORM nor AICC APIs could be found',
                );
                return;
              }
            }
    ...
    if (winDebug && !winDebug.closed) {
      if (!window.firstMsg) {
        ...
        winDebug.document.write(
          "<div class='debugLog' style='background-color:" +
            getBackgroundColorForLogs() +
            ";padding-bottom: 10px;font-family:monospace;word-wrap: break-word;'>" +
            strLine +
            '</div>\n',
        );
      }
...
function ShowDebugWindow(lmsPreview) {
  if (!lmsPreview) {
    ...
    winDebug = window.parent.window.open(
      '',
      'Debug',
...
function RecordNumericInteraction(
  ...
  if (
    strCorrectResponse != undefined &&
    strCorrectResponse != null &&
    IsValidDecimal(strCorrectResponse) == false
  ) {
    WriteToDebug('ERROR - Invalid Correct Response, not a valid decmial');
    SetErrorInfo(
      ERROR_INVALID_NUMBER,
      'Invalid Correct Response passed to RecordNumericInteraction (not a valid decimal), strCorrectResponse=' +
        strCorrectResponse,
    );
    return false;
  }
...
function createButtonToggleEvent() {
  document.getElementById('toggleDebugWindowButton').onclick =
    toggleDebugWindow;
}
function toggleDebugWindow() {
  ...
  if (debugIframeDiv.style.display == 'block') {
    ...
    window.parent.document.getElementById('toggleDebugWindowButton').innerHTML =
      'Display Debug logs';
  } else {
    debugIframeDiv.style.display = 'block';
    document.getElementById('toggleDebugWindowButton').innerHTML =
      'Hide Debug logs';
  }
}
function SizeDisplay() {
  var iframeTag = getDebugLogsIFrame();
  var debugDoc = iframeTag.contentWindow.document;
  debugDoc.getElementById('myButton').innerHTML = String(
...
function getDebugLogsIFrame() {
  ...
  if (lCurrentWindow) {
    while (1) {
      ...
      if (lParentWindow == lCurrentWindow) return '';
      if (lParentWindow == undefined) return '';
      lCurrentWindow = lParentWindow;
      lNoOfLoops += 1;
      if (lNoOfLoops > 10) return '';
    }
  }
}
